...
<div class="cover"><h1>QA Test Document</h1><div class="badge" style="background:${c}">Owner (เจ้าของร้าน)</div><div class="sub">QRforPay POS — S-01, S-03, S-04, S-12</div><div class="date">Generated 19/03/2026 (CE) | QA — verified from source code</div></div>
...
${sc('S01-07-paywall-setupfee.png', 'Paywall — ค่าแรกเข้า ฿1,399')}

<h4 style="color:#1e40af;margin:8px 0 4px;">Trial + Paywall Conditions (จาก SubscriptionGuard.tsx)</h4>
<div class="note i"><strong>หลักการ:</strong> setup_fee_paid=false = Trial user / setup_fee_paid=true = สมาชิกจ่ายแล้ว — admin สามารถยืดเวลา trial ได้ผ่าน subscription_paid_until โดยไม่ต้องเปลี่ยน setup_fee_paid</div>
<table>
<tr><th>สถานะ</th><th>Condition (code)</th><th>ผลลัพธ์</th></tr>
<tr><td>ร้านถูกลบ (soft delete)</td><td><span class="mono">shop.is_deleted === true</span></td><td><span class="tag r">BLOCKED</span> "ร้านค้าถูกระงับ" + mailto contact.runawaytech@gmail.com</td></tr>
<tr><td>ยังไม่มีสินค้า</td><td><span class="mono">!shop.first_product_at</span></td><td><span class="tag g">OK</span> ใช้ได้ไม่จำกัด (trial ยังไม่เริ่ม)</td></tr>
<tr><td>Trial ปกติ เหลือ 4-7 วัน</td><td><span class="mono">!setupFeePaid && !subDate && trialDaysLeft >= 4</span></td><td><span class="tag g">OK</span> ใช้งานปกติ ไม่แสดงอะไร</td></tr>
<tr><td>Trial ปกติ เหลือ 1-3 วัน</td><td><span class="mono">!setupFeePaid && !subDate && trialDaysLeft 1-3</span></td><td><span class="tag y">Banner</span> "ทดลองใช้ฟรีเหลืออีก X วัน — หลังจากนั้นต้องชำระค่าแรกเข้า ฿1,399"</td></tr>
<tr><td>Trial ปกติ หมดแล้ว</td><td><span class="mono">trialExpiredNaturally</span></td><td><span class="tag r">BLOCKED</span> Paywall ฿1,399 ทันที — ไม่มี grace period</td></tr>
<tr><td>Admin ยืด trial (subDate set) — เหลือ 1-2 วัน</td><td><span class="mono">!setupFeePaid && subDate && daysUntilExpiry 0-2</span></td><td><span class="tag y">Banner</span> "การทดลองใช้ฟรีจะหมดอายุในอีก X วัน / หมดอายุวันนี้"</td></tr>
<tr><td>Admin ยืด trial — หมดแล้ว (daysOverdue > 0)</td><td><span class="mono">trialExpiredByExtension</span></td><td><span class="tag r">BLOCKED</span> Paywall ฿1,399 ทันที — ไม่มี grace period</td></tr>
<tr><td>สมาชิก (setup_fee_paid=true) วันหมดอายุ</td><td><span class="mono">setupFeePaid && daysOverdue === 0</span></td><td><span class="tag g">OK</span> ใช้ได้ปกติทั้งวัน</td></tr>
<tr><td>สมาชิก เกิน 1-2 วัน</td><td><span class="mono">setupFeePaid && daysOverdue 1-2</span></td><td><span class="tag y">Modal</span> เตือนจ่ายค่ารายเดือน ฿199 (dismiss ได้วันละ 1 ครั้ง via localStorage)</td></tr>
<tr><td>สมาชิก เกิน 3+ วัน</td><td><span class="mono">setupFeePaid && daysOverdue >= 3</span></td><td><span class="tag r">BLOCKED</span> Paywall ฿199 + QR + ปุ่ม "ชำระแล้ว (ต่ออายุ 1 เดือน)"</td></tr>
<tr><td>จ่าย ฿1,399 (QR) หรือ referral (ผ่านตัวแทน ฿999)</td><td><span class="mono">setup_fee_paid = true</span></td><td><span class="tag g">AUTO</span> ระบบตั้ง subscription_paid_until = +1 เดือนปฏิทิน อัตโนมัติ</td></tr>
</table>

<h4 style="color:#1e40af;margin:8px 0 4px;">หน้า BLOCKED (เกิน 3 วัน) — แสดงอะไรบ้าง</h4>
<table>
<tr><th>Element</th><th>Detail</th></tr>
<tr><td>Icon</td><td>⚠️ AlertTriangle สีแดง</td></tr>
<tr><td>Title</td><td>"ไม่สามารถใช้ระบบได้"</td></tr>
<tr><td>Subtitle</td><td>"กรุณาชำระค่าบริการรายเดือนเพื่อใช้งานต่อ"</td></tr>
<tr><td>ชื่อร้าน</td><td>shop.name</td></tr>
<tr><td>แพ็คเกจ</td><td>"Pro (รายเดือน)"</td></tr>
<tr><td>วันหมดอายุ</td><td>subscription_paid_until (dd/mm/yyyy) + "(เกิน X วัน)" สีแดง</td></tr>
<tr><td>จำนวนเงิน</td><td>"ค่าบริการ ฿199/เดือน"</td></tr>
<tr><td>QR Code</td><td>PromptPay QR ฿199 (company PromptPay)</td></tr>
<tr><td>PromptPay</td><td>เบอร์ของ super admin</td></tr>
<tr><td>Button</td><td>"ชำระแล้ว (ต่ออายุ 1 เดือน)" → คำนวณวันหมดอายุใหม่ตาม calcNewExpiry</td></tr>
</table>

<h4 style="color:#1e40af;margin:8px 0 4px;">คำนวณวันต่ออายุ — calcNewExpiry (จาก SubscriptionGuard.tsx)</h4>
<table>
<tr><th>เกินกำหนดกี่วัน</th><th>ถ้าจ่ายวันนี้</th><th>Logic</th></tr>
<tr><td><strong>0-10 วัน</strong></td><td>นับต่อจากวันหมดอายุเดิม +1 เดือนปฏิทิน</td><td><span class="mono">addOneMonth(originalExpiry)</span><br>เช่น หมดอายุ 17/03 → จ่ายวันที่ 20/03 → หมดอายุใหม่: 17/04</td></tr>
<tr><td><strong>11+ วัน</strong></td><td>เริ่มนับใหม่จากวันจ่าย +1 เดือนปฏิทิน</td><td><span class="mono">addOneMonth(today)</span><br>เช่น หมดอายุ 17/03 → จ่ายวันที่ 30/03 → หมดอายุใหม่: 30/04</td></tr>
<tr><td><strong>ไม่มีวันหมดอายุเดิม</strong></td><td>เริ่มจากวันจ่าย +1 เดือนปฏิทิน</td><td><span class="mono">addOneMonth(today)</span></td></tr>
</table>
<div class="note w">เหตุผลที่แบ่ง 10 วัน: จ่ายช้าไม่เกิน 10 วัน = ยุติธรรม ไม่เสียวัน / จ่ายช้าเกิน 10 วัน = เริ่มนับใหม่ ไม่ได้วันเพิ่มย้อนหลัง</div>

<h4 style="color:#1e40af;margin:8px 0 4px;">Timing — สมาชิก (setup_fee_paid=true) หมดอายุ 17/03</h4>
<table>
<tr><th>วัน</th><th>ตัวอย่าง</th><th>สถานะ</th><th>ถ้าจ่ายวันนี้</th></tr>
<tr><td>วันหมดอายุ</td><td>17/03</td><td>✅ ใช้ได้ปกติทั้งวัน</td><td>หมดอายุใหม่: 17/04</td></tr>
<tr><td>+1 วัน</td><td>18/03</td><td>⚠️ Modal เตือน (dismiss ได้)</td><td>หมดอายุใหม่: 17/04</td></tr>
<tr><td>+2 วัน</td><td>19/03</td><td>⚠️ Modal เตือน (dismiss ได้)</td><td>หมดอายุใหม่: 17/04</td></tr>
<tr><td>+3 วัน</td><td>20/03</td><td>❌ BLOCKED + QR ฿199</td><td>หมดอายุใหม่: 17/04</td></tr>
<tr><td>+11 วันขึ้นไป</td><td>28/03+</td><td>❌ BLOCKED + QR ฿199</td><td>จ่าย 28/03 → หมดอายุ 28/04 (เริ่มนับใหม่)</td></tr>
</table>

<h4 style="color:#1e40af;margin:8px 0 4px;">Timing — Trial ที่ admin ยืดเวลา (setup_fee_paid=false) หมดอายุ 17/03</h4>
<table>
<tr><th>วัน</th><th>ตัวอย่าง</th><th>สถานะ</th></tr>
<tr><td>-2 วันก่อนหมด</td><td>15/03</td><td>🟡 Banner "การทดลองใช้ฟรีจะหมดอายุในอีก 2 วัน"</td></tr>
<tr><td>-1 วันก่อนหมด</td><td>16/03</td><td>🟡 Banner "การทดลองใช้ฟรีจะหมดอายุในอีก 1 วัน"</td></tr>
<tr><td>วันหมดอายุ</td><td>17/03</td><td>🟡 Banner "การทดลองใช้ฟรีหมดอายุวันนี้"</td></tr>
<tr><td>+1 วัน</td><td>18/03</td><td>❌ BLOCKED + Paywall ฿1,399 ทันที (ไม่มี grace)</td></tr>
</table>
<div class="note i">ระบบเช็คเฉพาะวันที่ (ไม่เช็คเวลา) — เปลี่ยนสถานะตอนเที่ยงคืนของแต่ละวัน</div>

<h4 style="color:#1e40af;margin:8px 0 4px;">Paywall แสดงรายละเอียดอะไรบ้าง</h4>
<table>
<tr><th>ข้อมูล</th><th>แสดง</th></tr>
<tr><td>ชื่อร้าน</td><td>shop.name</td></tr>
<tr><td>แพ็คเกจ</td><td>"Pro (รายเดือน)"</td></tr>
<tr><td>วันหมดอายุ</td><td>subscription_paid_until (dd/mm/yyyy)</td></tr>
<tr><td>จำนวนวันเกิน</td><td>"เกิน X วัน"</td></tr>
<tr><td>QR PromptPay</td><td>เบอร์ของ super admin (จาก DB function get_company_promptpay)</td></tr>
<tr><td>จำนวนเงิน</td><td>฿199 (รายเดือน) หรือ ฿1,399 (แรกเข้า เว็บ) / ฿999 (ผ่านตัวแทน)</td></tr>
</table>

<h4 style="color:#1e40af;margin:8px 0 4px;">Referral Code Validation (บนหน้า paywall ฿1,399)</h4>
<table>
<tr><th>กรอก</th><th>เช็คกับ</th><th>ผลลัพธ์</th></tr>
<tr><td>ว่าง</td><td>-</td><td>"กรุณากรอกรหัสตัวแทน"</td></tr>
<tr><td>Code ไม่มีใน DB</td><td><span class="mono">agents.code = CODE AND active = true</span></td><td>"รหัสตัวแทนไม่ถูกต้อง"</td></tr>
<tr><td>Code ถูก แต่ active=false</td><td>agents.active = false</td><td>"รหัสตัวแทนไม่ถูกต้อง"</td></tr>
<tr><td>Code ถูก + active=true</td><td>✅ Found</td><td>shops: setup_fee_paid=true, referral_code=CODE → ปิด paywall ทันที</td></tr>
</table>

<h4 style="color:#1e40af;margin:8px 0 4px;">Constants (from code)</h4>
<table>
<tr><th>Constant</th><th>Value</th></tr>
<tr><td><span class="mono">TRIAL_DAYS</span></td><td>7</td></tr>
<tr><td><span class="mono">SETUP_FEE</span></td><td>1399 (สมัครผ่านเว็บ) / 999 (ผ่านตัวแทน)</td></tr>
<tr><td><span class="mono">MONTHLY_FEE</span></td><td>199</td></tr>
</table>

<h4 style="color:#1e40af;margin:8px 0 4px;">Trial Calculation (code)</h4>
<div class="pre">getTrialDaysLeft(shop):<br>
- if (!shop.first_product_at) return -1 // unlimited, no trial started<br>
- expiry = first_product_at + 7 days<br>
- return Math.max(0, Math.ceil((expiry - now) / 1day))</div>

<h4 style="color:#1e40af;margin:8px 0 4px;">Paywall — Setup Fee Screen</h4>
<table>
<tr><th>Element</th><th>Detail</th></tr>
<tr><td>Title</td><td>"หมดระยะทดลองใช้ฟรี"</td></tr>
<tr><td>QR Code</td><td>PromptPay QR ฿1,399 (company PromptPay from super_admin profile.pending_promptpay)</td></tr>
<tr><td>Button</td><td>"โอนแล้ว" → marks setup_fee_paid=true</td></tr>
<tr><td>Divider</td><td>"หรือ"</td></tr>
<tr><td>Referral field</td><td>"รหัสตัวแทน (ชำระผ่านตัวแทนแล้ว)" + input + check button</td></tr>
<tr><td>Referral validation</td><td>Checks agents table: code matches + active=true<br>Error: "กรุณากรอกรหัสตัวแทน" (empty) / "รหัสตัวแทนไม่ถูกต้อง" (not found)</td></tr>
</table>

<h4 style="color:#1e40af;margin:8px 0 4px;">Monthly Reminder Modal</h4>
<table>
<tr><th>Element</th><th>Detail</th></tr>
<tr><td>Title</td><td>"กรุณาชำระค่าบริการ"</td></tr>
<tr><td>Subtitle</td><td>"ค่าบริการรายเดือน ฿199"</td></tr>
<tr><td>Warning</td><td>"เกินกำหนด {daysOverdue} วัน — ระบบจะถูกระงับใน {3 - daysOverdue} วัน"</td></tr>
<tr><td>Buttons</td><td>"ภายหลัง" (dismiss) / "ชำระเงิน" (show QR)</td></tr>
<tr><td>Dismiss</td><td>localStorage key: <span class="mono">qrforpay_sub_dismissed_{shopId}_{YYYY-MM-DD}</span> — once per day</td></tr>
</table>
</div>
...
<table>
<tr><th>Condition</th><th>แสดงผล</th><th>สี</th></tr>
<tr><td><span class="mono">!subDate && !setup_fee_paid && !first_product_at</span></td><td>"ทดลองใช้ฟรี 7 วัน — เริ่มนับหลังสร้างสินค้าชิ้นแรก"</td><td>Blue</td></tr>
<tr><td><span class="mono">!subDate && !setup_fee_paid && isInTrial</span></td><td>"ทดลองใช้ฟรี" + "หมดอายุ {date en-GB} (เหลือ X วัน)"</td><td>Amber</td></tr>
<tr><td><span class="mono">subDate && !subExpired && !setup_fee_paid</span></td><td>"ทดลองใช้ฟรี" + "ถึง {date en-GB}"</td><td>Green</td></tr>
<tr><td><span class="mono">subDate && !subExpired && setup_fee_paid</span></td><td>"สมาชิก" + "ถึง {date en-GB}"</td><td>Green</td></tr>
<tr><td><span class="mono">subDate && subExpired</span></td><td>"หมดอายุแล้ว" + "ถึง {date en-GB}"</td><td>Red</td></tr>
</table>
<div class="note i">subExpired เปรียบวันชนวัน (setHours 0,0,0,0) — หมดอายุตอนเที่ยงคืน ใช้ได้ตลอดวันหมดอายุ</div>
<div class="note w">Admin ยืดเวลา trial → label ยังเป็น "ทดลองใช้ฟรี" (setup_fee_paid ยังเป็น false) จนกว่า owner จะจ่าย ฿1,399 (เว็บ) หรือผ่านตัวแทน ฿999</div>
<div class="note w">Dates ใช้ en-GB format (DD/MM/YYYY) ไม่ใช่ th-TH (BE year)</div>
</div>
...
function customerQA(): string {
  const c = '#0d9488'
  return `<!DOCTYPE html><html lang="th"><head><meta charset="UTF-8"><style>${CSS}</style></head><body>
<div class="cover"><h1>QA Test Document</h1><div class="badge" style="background:${c}">Customer (ลูกค้า)</div><div class="sub">QRforPay POS — S-05, S-06, S-07</div><div class="date">Generated 19/03/2026 (CE) | QA — verified from source code</div></div>
...
function cashierQA(): string {
  const c = '#0891b2'
  return `<!DOCTYPE html><html lang="th"><head><meta charset="UTF-8"><style>${CSS}</style></head><body>
<div class="cover"><h1>QA Test Document</h1><div class="badge" style="background:${c}">Cashier (แคชเชียร์)</div><div class="sub">QRforPay POS — S-08, S-09, S-10, S-11</div><div class="date">Generated 19/03/2026 (CE) | QA — verified from source code</div></div>
...
function adminQA(): string {
  const c = '#16a34a'
  return `<!DOCTYPE html><html lang="th"><head><meta charset="UTF-8"><style>${CSS}</style></head><body>
<div class="cover"><h1>QA Test Document</h1><div class="badge" style="background:${c}">Super Admin</div><div class="sub">QRforPay POS — S-02 + RBAC + Subscription</div><div class="date">Generated 19/03/2026 (CE) | QA — verified from source code</div></div>
...
<tr><td>Min date</td><td><span class="mono">min={new Date().toISOString().slice(0, 10)}</span> — today (cannot pick past dates)</td></tr>
...
<div class="desc">Complete paywall logic from SubscriptionGuard.tsx — all conditions</div>

<h4 style="color:#1e40af;margin:8px 0 4px;">Constants</h4>
<table>
<tr><th>Constant</th><th>Value</th><th>Usage</th></tr>
<tr><td><span class="mono">TRIAL_DAYS</span></td><td>7</td><td>Free trial period after first product</td></tr>
<tr><td><span class="mono">SETUP_FEE</span></td><td>1399 (สมัครผ่านเว็บ) / 999 (ผ่านตัวแทน)</td><td>One-time setup fee (or bypass with referral code)</td></tr>
<tr><td><span class="mono">MONTHLY_FEE</span></td><td>199</td><td>Monthly subscription fee</td></tr>
</table>

<h4 style="color:#1e40af;margin:8px 0 4px;">Trial Logic</h4>
<table>
<tr><th>Condition</th><th>Trial Status</th><th>Behavior</th></tr>
<tr><td><span class="mono">!shop.first_product_at</span></td><td>Not started</td><td><span class="tag g">UNLIMITED</span> — no trial yet, use freely. getTrialDaysLeft() returns -1</td></tr>
<tr><td><span class="mono">first_product_at</span> exists + within 7 days</td><td>Active trial</td><td><span class="tag g">OK</span> — trial running. Days left calculated from first_product_at + 7 days</td></tr>
<tr><td>Trial 1-3 days left + !setup_fee_paid</td><td>Ending soon</td><td><span class="tag y">BANNER</span> "ทดลองใช้ฟรีเหลืออีก X วัน — หลังจากนั้นต้องชำระค่าแรกเข้า ฿1,399"</td></tr>
<tr><td>Trial 0 days left + !setup_fee_paid</td><td>Expired</td><td><span class="tag r">BLOCKED</span> — setup fee paywall (QR ฿1,399 or referral code ฿999 via agent)</td></tr>
</table>

<h4 style="color:#1e40af;margin:8px 0 4px;">Post-Trial Paywall</h4>
<table>
<tr><th>Condition</th><th>Screen</th><th>User Action</th></tr>
<tr><td>trialExpired + !setupFeePaid</td><td>"หมดระยะทดลองใช้ฟรี" + QR ฿1,399</td><td>1) Scan QR → pay ฿1,399 → press "โอนแล้ว"<br>2) Enter referral code (agent path, ฿999 to agent)<br>→ sets shop.setup_fee_paid=true</td></tr>
<tr><td>setupFeePaid + daysOverdue 1-2</td><td>Modal: "กรุณาชำระค่าบริการ" + "฿199/เดือน"</td><td>Dismiss (once/day via localStorage) or show QR ฿199<br>"เกินกำหนด X วัน — ระบบจะถูกระงับใน {3-X} วัน"</td></tr>
<tr><td>setupFeePaid + daysOverdue 3+</td><td>"ไม่สามารถใช้ระบบได้" + QR ฿199</td><td>กดปุ่ม "ชำระแล้ว (ต่ออายุ 1 เดือน)" → ระบบต่ออายุอัตโนมัติ (≤10 วัน: นับจากเดิม, &gt;10 วัน: นับจากวันจ่าย)</td></tr>
<tr><td>shop.is_deleted</td><td>"ร้านค้าถูกระงับ" + contact email</td><td>Contact admin to undelete shop</td></tr>
</table>

<h4 style="color:#1e40af;margin:8px 0 4px;">Referral Code Validation (on paywall)</h4>
<table>
<tr><th>Input</th><th>Check</th><th>Error Message</th></tr>
<tr><td>Empty</td><td>-</td><td>"กรุณากรอกรหัสตัวแทน"</td></tr>
<tr><td>Code not in agents table</td><td><span class="mono">agents.code = CODE AND active = true</span></td><td>"รหัสตัวแทนไม่ถูกต้อง"</td></tr>
<tr><td>Valid code</td><td>Found in agents</td><td>Updates shop: setup_fee_paid=true, referral_code=CODE</td></tr>
</table>

<h4 style="color:#1e40af;margin:8px 0 4px;">Days Overdue Calculation (code)</h4>
<div class="pre">getDaysOverdue(shop):<br>
- if (!shop.subscription_paid_until) return 0<br>
- paidUntil = new Date(subscription_paid_until), today = new Date()<br>
- both setHours(0,0,0,0)<br>
- return Math.max(0, Math.floor((today - paidUntil) / 1day))</div>

<div class="note w">Monthly reminder modal uses localStorage key: <span class="mono">qrforpay_sub_dismissed_{shopId}_{YYYY-MM-DD}</span> — dismissed once per day per shop</div>
<div class="note i">Company PromptPay for paywall QR codes is fetched from super_admin profile.pending_promptpay field (default fallback: 0994569544)</div>
</div>
...
function agentQA(): string {
  const c = '#d97706'
  return `<!DOCTYPE html><html lang="th"><head><meta charset="UTF-8"><style>${CSS}</style></head><body>
<div class="cover"><h1>QA Test Document</h1><div class="badge" style="background:${c}">Agent (ตัวแทนขาย)</div><div class="sub">QRforPay POS — ระบบตัวแทนขาย + Referral</div><div class="date">March 2026 | QA Professional — ตรวจสอบจากโค้ดจริงทุก condition</div></div>
...
${SL('5', c, 'สมัครสำเร็จ → ไม่ต้องจ่าย ฿1,399 ให้ระบบ', 'self_register_shop RPC ส่ง p_referral_code=CODE → shop.referral_code=CODE, setup_fee_paid=true (bypass) — ตัวแทนรับ ฿999 โดยตรงจากร้าน')}
