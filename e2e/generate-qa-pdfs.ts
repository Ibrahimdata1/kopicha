...
<tr><td>Cashier</td><td>/pos/sessions, /pos/orders, /pos/tables, /pos/settings (read-only + ชำระเงิน)</td></tr>
</table>
</div>
</div>
`
...
function cashierQA(): string {
  const c = '#0891b2'
  return `<!DOCTYPE html><html lang="th"><head><meta charset="UTF-8"><style>${CSS}</style></head><body>
<div class="cover"><h1>QA Test Document</h1><div class="badge" style="background:${c}">Cashier (แคชเชียร์)</div><div class="sub">QRforPay POS — S-08, S-09, S-10, S-11, S-12</div><div class="date">Generated 21/03/2026 (CE) | QA — verified from source code</div></div>
${CREDS_PAGE}
...
<div class="note w">Cashier เห็นแท็บ: โต๊ะ, บิล, ตั้งค่า — ไม่เห็น สินค้า, สรุป<br>หน้าตั้งค่า: ดูข้อมูลร้าน + ทีมได้ (read-only) + ชำระค่าบริการแทน owner ได้</div>
</div>
...
<!-- ═══ S-12 Cashier Settings (Read-only + Payment) ═══ -->
<div class="page">
<h2><span class="num" style="background:#0891b2">S-12</span> แคชเชียร์ดูตั้งค่า + ชำระเงินแทน Owner</h2>
<div class="pre">Precondition: Cashier login, nav bar เห็นแท็บ "ตั้งค่า"</div>

${S('1', '#0891b2', 'กดแท็บ "ตั้งค่า" → /pos/settings', 'เห็น 3 sections: สถานะสมาชิก, ข้อมูลร้าน (read-only), ทีมงาน (read-only)')}
${sc('S12-1-cashier-settings-full.png', 'หน้าตั้งค่า Cashier — read-only + QR ชำระเงิน')}

<h4 style="color:#1e40af;margin:8px 0 4px;">สิทธิ์ Cashier ในหน้าตั้งค่า</h4>
<table>
<tr><th>Section</th><th>Owner</th><th>Cashier</th></tr>
<tr><td><strong>สถานะสมาชิก</strong></td><td>✅ ดู + QR ชำระเงิน</td><td>✅ ดู + QR ชำระเงิน (เหมือนกัน)</td></tr>
<tr><td><strong>ข้อมูลร้าน</strong> (ชื่อร้าน, PromptPay, โต๊ะ, ระบบจ่ายเงิน)</td><td>✅ แก้ไข + บันทึก</td><td>✅ <strong>อ่านอย่างเดียว</strong> — ไม่มีปุ่มบันทึก</td></tr>
<tr><td><strong>ทีม — รายชื่อ</strong></td><td>✅ ดู + รีเซ็ต PW / ลบ</td><td>✅ ดูรายชื่อ — <strong>ไม่มีปุ่ม action</strong></td></tr>
<tr><td><strong>ทีม — รหัสผ่าน cashier</strong></td><td>✅ ดู/copy PW</td><td>❌ ไม่เห็น</td></tr>
<tr><td><strong>ทีม — เพิ่มพนักงาน</strong></td><td>✅ ฟอร์ม add cashier</td><td>❌ ไม่เห็น</td></tr>
</table>

${S('2', '#0891b2', 'ชำระเงินแทน Owner', 'กด "แสดง QR" → สร้าง charge → QR + countdown 5 นาที')}
${sc('S12-2-cashier-settings-top.png', 'Nav bar Cashier — เห็นแท็บ ตั้งค่า')}

<h4 style="color:#1e40af;margin:8px 0 4px;">QR Payment Flow (ทั้ง Owner + Cashier)</h4>
<table>
<tr><th>สถานะ</th><th>UI แสดง</th><th>Action</th></tr>
<tr><td><span class="tag g">สำเร็จ</span></td><td>"ชำระสำเร็จ!" + ✅</td><td>Reload อัตโนมัติใน 1.5 วิ</td></tr>
<tr><td><span class="tag r">ไม่สำเร็จ</span></td><td>"การชำระเงินไม่สำเร็จ"</td><td>ปุ่ม "สร้าง QR ใหม่" → สร้าง charge ใหม่ทันที</td></tr>
<tr><td><span class="tag" style="background:#f59e0b">หมดเวลา</span></td><td>"QR หมดเวลา" (5 นาที)</td><td>ปุ่ม "สร้าง QR ใหม่" → สร้าง charge ใหม่ทันที</td></tr>
</table>

<div class="note s">Cashier ชำระเงินต่ออายุแทน Owner ได้ — ไม่ต้อง logout ไปใช้ไอดี Owner<br>เพราะ QR charge ผูกกับ shop_id ไม่ใช่ user_id</div>
<div class="note i">ระบบ poll สถานะ charge จาก Omise API ทุก 4 วินาที — ถ้า fail/expired/reversed แสดงผลทันที</div>
</div>
</body></html>`
}
