...
export default function BillsPage() {
  ...
  useEffect(() => { if (shop?.id) fetchBills() }, [shop?.id, fetchBills])
  ...
  return (
    <div className="max-w-4xl mx-auto px-4 py-6">
      <div className="page-header">
        ...
        <button onClick={fetchBills} className="btn-secondary px-3 py-1.5 text-sm">
          ↻ รีเฟรช
        </button>
      </div>
          ...
          >
            ...
            {filter !== tab.key && counts[tab.key] > 0 && (
              <span className={`text-xs px-1.5 py-0.5 rounded-full ${
                filter === tab.key ? 'bg-white/20' : 'bg-slate-100 dark:bg-slate-700 text-slate-500 dark:text-slate-400'
              }`}>
                {counts[tab.key]}
              </span>
            )}
              ...
              <div key={bill.id} className="flex items-center gap-4 px-5 py-4 hover:bg-slate-50/80 dark:hover:bg-slate-700/30 transition-colors">
                ...
                <div className="flex-1 min-w-0">
                  ...
                  <p className="text-xs text-subtle font-mono">
                    {bill.id.slice(0, 12)}... · {timeAgo(bill.created_at)}
                  </p>
                </div>
