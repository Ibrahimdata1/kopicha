      ...
      <div className="flex items-center justify-center h-64">
        <div className="text-center space-y-3">
          ...
          <p className="text-sm text-muted">
            {isSuperAdmin
              ? 'ไปที่หน้า Admin เพื่อจัดการร้านค้าทั้งหมด'
              : 'กรุณาตั้งค่าร้านค้าที่เมนู ตั้งค่า'}
          </p>
          {isSuperAdmin && (
            <a href="/pos/admin" className="btn-primary inline-flex px-4 py-2 text-sm">
              ไปหน้า Admin
            </a>
          )}
        </div>
    ...
    <div className="max-w-6xl mx-auto px-4 py-6">
      ...
      <div className="page-header">
        ...
        <div className="flex gap-2">
          <a href="/pos/sessions/history" className="btn-secondary px-4 py-2 text-sm">
            ประวัติ
          </a>
      ...
      {sessions.length === 0 ? (
        <div className="bg-white dark:bg-slate-800 border border-dashed border-slate-200 dark:border-slate-700 rounded-2xl p-16 text-center">
          ...
          <button onClick={() => setShowGenerate(true)} className="btn-primary mt-6 px-6 py-2.5 text-sm">
            + บิลใหม่
          </button>
        </div>
      ) : (
        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 gap-3">
          {sessions.map((session) => {
            ...
            const itemCount = session.orders.reduce(
              (sum, o) => sum + (o.items ?? []).filter((i) => (i.item_status ?? 'active') === 'active').length,
              0
            )
            return (
              <button
